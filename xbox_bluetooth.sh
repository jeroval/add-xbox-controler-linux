...
################################################################################
# xbox_bluetooth_auto_connect.sh
# Version : 2.0
# Description : Script intelligent et autonome pour connecter automatiquement une
#               manette Xbox Bluetooth sous Linux (Debian, Ubuntu, Mint, Zorin).
################################################################################
...
check_dependencies() {
  local deps=(bluetoothctl rfkill systemctl modprobe grep sed awk sleep mkdir tee)
  for dep in "${deps[@]}"; do
...
log INFO "====== DÉMARRAGE DU SCRIPT xbox_bluetooth_auto_connect.sh ======"
check_dependencies
configure_main_conf
disable_ertm_if_needed
start_bluetooth
